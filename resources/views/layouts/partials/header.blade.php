    ...
    <div class="flex items-center gap-3 sm:gap-4">
        ...
        <div x-data="{ dropdownOpen: false }" class="relative">
            ...
            <button @click="dropdownOpen = !dropdownOpen" class="relative block h-10 w-10 rounded-full overflow-hidden border-2 border-gray-300 focus:outline-none focus:border-blue-500 dark:border-gray-600">
                ...
                src="{{ Auth::user()->karyawan->file?asset('storage/' . Auth::user()->karyawan->file):asset('images/d_avatar.png') }}"
                alt="Foto Profil">
            </button>
            ...
            <div x-show="dropdownOpen" 
                ...
                <div class="flex items-center px-4 py-3 border-b border-gray-100 dark:border-gray-700">
                    ...
                    >
                    <div>
                        <p class="text-sm font-semibold text-gray-800 dark:text-white">{{ Auth::user()->karyawan->nama??'Not Found' }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">{{ Auth::user()->email??'Not Found' }}</p>
                        <p class="text-xs text-gray-500 dark:text-gray-400">Sebagai : {{ Auth::user()->otoritas??'Not Found' }}</p>
                    </div>
                </div>
