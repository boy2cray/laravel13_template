        ...
        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            ...
            <tbody class="bg-white divide-y divide-gray-100 dark:bg-gray-900 dark:divide-gray-800">
                @forelse ($konten as $item)
                    <tr wire:key="row-{{ data_get($item,'id',$loop->index) }}" class="hover:bg-blue-100 transition-colors duration-200 dark:hover:bg-gray-700/50">
                        @foreach ($headers as $header)
                            {{-- NO --}}
                            @if ($header['label'] === '#')
                            ...
                            @elseif ($header['label'] === 'Foto')
                                <td class="td w-20 text-center">
                                    @if (data_get($item,'foto'))
                                        <img src="{{ asset('storage/'.data_get($item,'foto')) }}"
